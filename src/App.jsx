...
function LoginPage({ login, toast$, go, t, lang }) {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const u = await dbIdx("users", "email", f.email.toLowerCase().trim());
      if (!u) { toast$(lang === "bn" ? "ইমেইল নিবন্ধিত নয়।" : "Email not registered.", "err"); setLoading(false); return; }
      const h = await hashPass(f.pass);
      if (u.passwordHash !== h) { toast$(lang === "bn" ? "পাসওয়ার্ড ভুল।" : "Wrong password.", "err"); setLoading(false); return; }
      await login(u); toast$(`${t.loginSuccess}, ${u.name}! 🎉`); go("predict");
...
function RegisterPage({ login, toast$, go, t, lang }) {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const ex = await dbIdx("users", "email", f.email.toLowerCase().trim());
      if (ex) { toast$(lang === "bn" ? "এই ইমেইল ইতিমধ্যে নিবন্ধিত।" : "Email already registered.", "err"); setLoading(false); return; }
      ...
      await dbPut("users", nu); await login(nu);
...
function ForgotPage({ toast$, go, t, lang }) {
  ...
  const send = async () => {
    ...
    setLoading(true);
    const u = await dbIdx("users", "email", email.toLowerCase().trim());
    if (!u) { toast$(lang === "bn" ? "ইমেইল নিবন্ধিত নয়।" : "Email not registered.", "err"); setLoading(false); return; }
    const token = Math.floor(100000 + Math.random() * 900000).toString();
    await dbPut("reset_tokens", { token, userId: u.id, expires: Date.now() + 15 * 60 * 1000, email: u.email });
    ls.s("bd_reset_token", token);
    toast$(`OTP: ${token} (${lang === "bn" ? "১৫ মিনিট" : "15 min"})`); setSent(true); setLoading(false);
  };
  return (
...
function ResetPage({ toast$, go, t, lang }) {
  ...
  const reset = async () => {
    ...
    if (f.pass !== f.confirm) { toast$(lang === "bn" ? "পাসওয়ার্ড মিলছে না।" : "Passwords don't match.", "err"); return; }
    setLoading(true);
    const rec = await dbGet("reset_tokens", f.otp.trim());
    if (!rec || Date.now() > rec.expires) { toast$(lang === "bn" ? "OTP সঠিক/মেয়াদোত্তীর্ণ নয়।" : "Invalid/expired OTP.", "err"); setLoading(false); return; }
    const u = await dbGet("users", rec.userId);
    await dbPut("users", { ...u, passwordHash: await hashPass(f.pass) });
    await dbDel("reset_tokens", f.otp.trim());
    toast$(lang === "bn" ? "পাসওয়ার্ড পরিবর্তিত হয়েছে! 🎉" : "Password changed! 🎉"); go("login"); setLoading(false);
  };
  return (
...
function VerifyPage({ user, updateUser, toast$, go, t, lang }) {
  ...
  const verify = async () => {
    ...
    if (code.trim() === user.otp) { await updateUser({ ...user, emailVerified: true, otp: "" }); toast$(lang === "bn" ? "ইমেইল যাচাই সফল! 🎉" : "Email verified! 🎉"); go("predict"); }
      ...
      <div className="profile-grid" style={{ display: "grid", gridTemplateColumns: "280px 1fr", gap: 20, maxWidth: 900, margin: "0 auto" }}>
        ...
        <div>
          ...
          <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "var(--r)", padding: 16 }}>
            ...
            {[["Engine", "IndexedDB"], ["Password", "SHA-256"], ["ID", user.id.slice(0, 8) + "…"]].map(([k, v]) => (
